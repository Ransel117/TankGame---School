...
const Fhealth = 200
export var Phealth = Fhealth

const ffuel = 200
export var Pfuel = ffuel

onready var player2 = get_parent().get_child(2)
onready var weaponctrl = get_parent().get_child(6).get_child(1).get_child(0).get_child(1)
...
func get_input():
	var left = Input.is_action_pressed("Pleft")
	var right = Input.is_action_pressed("Pright")

	var up = Input.is_action_pressed("Pup")
	var down = Input.is_action_pressed("Pdown")
	if go:
		if Pfuel > 0:
			if left:
				velocity.x = -100
...
#	Gets weapon selected, checks turn
	bullet = weaponctrl.weapon
	get_input()
	if !get_parent().eTurn:
		if get_parent().eTurn == false and shot == false:
			shooting()
		go = true
	else:
		Pfuel = ffuel
...
#B2
			if bullet == 2:
				...
				b2.gravity = gravity
				shot = true
				b2.connect("damage1", self, "_on_bullet2_hit")
				b2.connect("damage2", player2, "_on_bullet2_hit")
#B3
			elif bullet == 3:
				...
				b3.gravity = gravity
				shot = true
				b3.connect("damage1", self, "_on_bullet3_hit")
				b3.connect("damage2", player2, "_on_bullet3_hit")
#B4
			elif bullet == 4:
				...
				b4.gravity = gravity
				shot = true
				b4.connect("damage1", self, "_on_bullet4_hit")
				b4.connect("damage2", player2, "_on_bullet4_hit")
#B5
			elif bullet == 5:
				...
				b5.gravity = gravity
				shot = true
				b5.connect("damage1", self, "_on_bullet5_hit")
				b5.connect("damage2", player2, "_on_bullet5_hit")
#B6
			elif bullet == 6:
				...
				b6.gravity = gravity
				shot = true
				b6.connect("damage1", self, "_on_bullet6_hit")
				b6.connect("damage2", player2, "_on_bullet6_hit")
#B1
			else:
				bullet == 1
				...
				b1.gravity = gravity
				shot = true
				b1.connect("damage1", self, "_on_bullet1_hit")
				b1.connect("damage2", player2, "_on_bullet1_hit")
